package xyz.nahidalibrary.galaxy.user.config.nshiro.realm
...
import xyz.nahidalibrary.galaxy.user.config.nshiro.CurrentAccount
import xyz.nahidalibrary.galaxy.user.config.nshiro.PRINCIPAL_KEY
import xyz.nahidalibrary.galaxy.user.config.nshiro.token.JwtToken
import xyz.nahidalibrary.galaxy.user.exception.BizException
...
open class JwtRealm : AuthorizingRealm() {
  ...
  override fun doGetAuthenticationInfo(authToken: AuthenticationToken): AuthenticationInfo? {
    ...
    val wrapper = QueryWrapper<UserModel>().eq(UserModel::id.name, JwtUtils.getId(token))
